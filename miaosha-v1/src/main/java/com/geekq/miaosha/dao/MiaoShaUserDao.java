...
//    public void insertMiaoShaUser(MiaoshaUser miaoshaUser);
    ...
    @Insert("insert into miaosha_user (id_num,real_name,id , nickname ,password , salt ,head,register_date,last_login_date)value (#{idNum},#{realName},#{id},#{nickname},#{password},#{salt},#{head},#{registerDate},#{lastLoginDate}) ")
    @Options(useGeneratedKeys = true, keyProperty = "id", keyColumn = "id")
    public void insertMiaoShaUser(MiaoshaUser miaoshaUser);
